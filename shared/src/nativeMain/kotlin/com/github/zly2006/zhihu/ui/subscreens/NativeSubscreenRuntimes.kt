...
// 本 fork 通过 DeveloperSettingsRuntime 暴露运行时信息，没有单独的
// rememberDeveloperRuntimeInfo expect，因此上游实现降级为普通辅助函数。
private fun nativeNetworkStatus(): String = if (nativeIsDesktop) {
    "网络状态：桌面端使用系统网络"
} else {
    "网络状态：iOS 端使用系统网络"
}

private fun nativeDeveloperRuntimeInfo(): DeveloperRuntimeInfo = DeveloperRuntimeInfo(
...
actual fun rememberDeveloperSettingsRuntime(): DeveloperSettingsRuntime {
    ...
    return remember(accountStore) {
        DeveloperSettingsRuntime(
            cookies = { accountStore.load().cookies },
            networkStatus = { nativeNetworkStatus() },
            powerSaveModeText = { null },
            runtimeInfo = { nativeDeveloperRuntimeInfo() },
            verifyLogin = { cookies -> accountStore.verifyAndSave(cookies.toMutableMap()) },
            refreshToken = { }, // TODO: Kotlin/Native token 刷新
            saveCookies = { cookies ->
                val current = accountStore.load()
                accountStore.save(current.copy(login = true, cookies = cookies.toMutableMap()))
            },
            signedGet = { "Kotlin/Native 暂不支持" }, // TODO: Kotlin/Native signed GET
        )
    }
}
