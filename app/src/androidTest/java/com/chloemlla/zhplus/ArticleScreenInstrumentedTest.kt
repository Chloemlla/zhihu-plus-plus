...
class ArticleScreenInstrumentedTest {
    ...
    fun highlightedTextDrawsDashesAcrossEveryWrappedLine() {
        ...
        for (line in startLine..endLine) {
            val top = (layout.getLineBottom(line) - with(density) { 6.dp.toPx() }).toInt().coerceAtLeast(0)
            val bottom = (layout.getLineBottom(line) + with(density) { 2.dp.toPx() }).toInt().coerceAtMost(pixels.height - 1)
